...
export function loadMetadata(path: string): SyncMetadata {
  if (!existsSync(path)) return { ...DEFAULT_METADATA, factions: {} }
  try {
    const raw = readFileSync(path, 'utf-8')
    return JSON.parse(raw) as SyncMetadata
  } catch {
    return { ...DEFAULT_METADATA, factions: {} }
  }
}
...
export function hasChanged(stored: SyncMetadata, remoteCommitSha: string): boolean {
  return stored.commitSha !== remoteCommitSha
}
